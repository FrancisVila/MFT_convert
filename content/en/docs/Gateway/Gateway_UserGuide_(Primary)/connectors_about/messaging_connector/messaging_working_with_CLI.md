...
[How to link {{< Gateway/componentlongname  >}} and Axway Messaging](#How_to_create_connector)

[Details of {{< Gateway/componentlongname  >}}/Axway Messaging link management commands](#Connector_managment_details)
...
-   From an Axway Messaging server to Gateway (*IN* connector)
-   From Gateway to an Axway Messaging server (*OUT* connector)
...
1.  Create a proplist (Property List) that describes the messages received from the Axway Messaging server. Use the `peladm create_proplist` command.
...
1.  Create a Model for Transfers to Gateway with the following characteristics:
    -   *TO\_GTW* value for the protocol (-pr) attribute
    -   Specify a Property list in the `prop_list_name (-pln)` attribute
...
1.  Create an *IN* type Axway Messaging connector. Use the `peladm create_xmsctor` command.
...
1.  Create a Decision Rule with the following characteristics:
    -   *XMS* value for type\_of\_decision rule type (-type) attribute
    -   *MODEL* value for the `execution_type (-et)` attribute
...
1.  Create a proplist (Property List) that describes the messages received from Gateway. Use the `peladm create_proplist` command.
...
1.  Create a Model for Transfers to the Axway Messaging server.
    -   *FROM\_GTW* value for the `protocol (-pr)` attribute
    -   Specify a Property list in the `prop_list_name (-pln)` attribute
...
1.  Create an *OUT* type Axway Messaging connector. Use the `peladm create_xmsctor` command.
...
1.  Create an XMS-type Decision Rule, with the following characteristics:
    -   *XFER\_CHANGE\_STATE* value for the type attribute
    -   *TO\_XMS* value for the execution type (-et) value
...
1.  Create a Transfer Change State-type Rule Table. Use the `peladm create_ruletable` command.
...
## Details of {{< Gateway/componentlongname  >}} / Axway Messaging link management commands

The following online commands are available for managing links between Gateway and an Axway Messaging server:

-   [`peladm create_xmsctor`](#Create_Messaging_connector)
-   [`peladm update_xmsctor`](#Update_Messaging_connector)
-   [`peladm delete_xmsctor`](#Delete_Messaging_connector)
-   [`peladm create_proplist`](#Create_proplist)
-   [`peladm update_proplist`](#Update_proplist)
-   [`peladm delete_proplist`](#Delete_proplist)
-   [`peldsp display_xmsctor`](#Display_connector)
-   [`peldsp select_xmsctor`](#Select_Messaging_connector)
-   [`peldsp display_proplist`](#Display_properties_list)
-   [`peldsp select_proplist`](#Select_properties_list)
...
<table>
   ...
   <tbody>
      <tr>
         ...
         <td><p><code>peladm create_xmsctor</code> [parameters]</p>         </td>
      </tr>
      <tr>
...
<p>Repeat the command for each required connector creation.</p>         </td>
      ...
      <tr>
         ...
         <td><p><code>-name (-n)</code>: Enter a name for the Axway Messaging connector.</p>         </td>
      </tr>
      <tr>
         <td><p><code>-type (-t)</code>: Enter one of the following values to indicate the Axway Messaging connector type:</p>
<ul>
<li><strong>IN</strong>: for transmitting incoming file transfers to an Axway Messaging server message queue</li>
