...
Attach the following Decision Rules to the default Rule Table. In each Decision Rule, modify the criteria `trade service= trgt.*` to `trade service= *`:
...
In the TARGET2 configuration file located at `../qt2/config`, set the execution mode to simulation (SIM):
