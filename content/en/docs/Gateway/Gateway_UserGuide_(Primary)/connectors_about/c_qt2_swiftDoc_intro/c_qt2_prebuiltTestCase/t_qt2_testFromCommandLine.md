...
-   **-ml** is the name of the Model called to execute the test case  
    For the current set of test cases you always use the Model FROM\_BO\_TO\_SSP.
-   **-fc** is the path to the XML file that Gateway uses to generate the initial request to SSP.  
    Gateway requires the file `InitialRequest.xml` to generate the initial request for all test cases.
-   **-param1** is the test case number  
