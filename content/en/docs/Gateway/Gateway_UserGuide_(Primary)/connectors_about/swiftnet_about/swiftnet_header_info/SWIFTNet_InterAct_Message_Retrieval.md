...
Sent or received InterAct Store-and-Forward messages can be retrieved from the SWIFT network by initiating a retrieval request System Message, which has the request type "`xsys.015.001.01`".
...
-   Only if you have been granted the "`swift.snf.system"` / `"OnlineRetriever`" RBAC role are you authorized to send retrieval requests.
-   Optionally, you can request a retrieval report which provides information about failed retrievals.

### Retrieved message flow

**Gateway side**: The user sends a retrieval request which has the request type “`xsys.015.001.01`”, specifying in the payload data:
...
-   If the optional retrieval report has also been requested, then the retrieval report, which has the request type “`xsys.016.001.01`”, is queued to the same queue specified in the request payload.
...
In order to receive a previously sent or received message, you have to submit a retrieval request system message which has the request type "`xsys.015.001.01`".
...
-   When performing the transfer request for message retrieval, you have to overwrite the request type with "`xsys.015.001.01`".
...
-   **Payload data specific to retrieval request system message**  
    ...
    The `xsys.015.001.01` retrieval request supports various options that allow you to:
...
Retrieval request payload for receiving a single message, previously received, with a specific `SnFRef`:
...
Retrieval request payload for receiving a single message, previously sent, with a specific `SnFRef`:
...
Retrieval request payload for receiving multiple messages, based on a range of `Sw:SnFOutputSeq` - starting sequence number and ending sequence number:
...
You need to have a queue which can receive retrieved messages. It must be defined for service `“swift.snf.system (!p, !x)`”. Moreover, you must use an output channel for starting the session on the queue, which has the option to receive retrieved messages activated. This can be done from the output channel configuration, from the [UI or command line](../../swiftnet_input_channels/swiftnet_output_channels#swift_op_ch_paras) .
...
-   In additional parameters file, by saving information specific to a retrieved message (`Sw:RetrievedDescriptor`)
...
The retrieval reports, which have the request type “`xsys.016.001.01`”, are received by Gateway due to the existing system message trading partner “`SYSTEM_MESSAGE`”, which accepts all types of system messages. If the configuration changed meanwhile, ensure that you have a trading partner for receiving these types of system messages.
...
<li>No = not a retrieved message</li>
      ...
      <tr>
         <td><strong>Retrieval info
(check box)</strong>         </td>
         <td>In the additional parameters file, the sub-primitive xml specific to a retrieved message is saved every time an incoming retrieved message is received. The name of the xml element is "<code>Sw:RetrievedDescriptor</code>”.         </td>
      </tr>
   </tbody>
</table>
...
-   "`Y`" - indicates that the additional parameters file contains information specific to a retrieved message on the xml element "`SwRetrievedDescriptor`”. This element was extracted from the incoming request primitive XML. When a retrieved message is received, this parameter must be set to "Y".
-   "`N`" – indicates that the additional parameters file does not contain information specific to a retrieved message because it is a normal transfer.

`x_sw_retrieved_message`

-   "`N`" – normal message
-   "`Y`" – retrieved message

`x_sw_retrieved_message_direction`
...
<th class="HeadD-Column1-Header1"><p>Description</p>         </th>
   ...
   <tbody>
      <tr>
         <td><p><code>x_retrieval_info</code></p>         </td>
         <td><ul>
<li>"<code>Y</code>" - indicates that the additional parameters file contains information specific to a retrieved message on the xml element „<code>SwRetrievedDescriptor</code>”. This element was extracted from the incoming primitive XML request. When a retrieved message is received, this parameter must be set to "Y".</li>
<li>"<code>N</code>" – indicates that the additional parameters file does not contain information specific to a retrieved message because it is a normal transfer.</li>
</ul>         </td>
      </tr>
      <tr>
         <td><p><code>x_sw_retrieved_message</code></p>         </td>
         <td><ul>
<li>"<code>N</code>" – normal message</li>
<li>"<code>Y</code>" – retrieved message</li>
</ul>         </td>
      </tr>
      <tr>
         <td><p><code>x_sw_retrieved_message_direction</code></p>         </td>
         <td><ul>
...
-   **sw retrieved message** Type: enum (Y, N).

This corresponds to the SWIFTNet `sw_retrieved_message` transfer parameter, set after a message has been received.

-   **sw retrieved message direction** Type: enum (sent, received).
