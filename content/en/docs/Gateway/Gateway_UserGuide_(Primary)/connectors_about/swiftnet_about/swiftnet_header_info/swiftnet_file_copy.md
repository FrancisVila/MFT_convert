...
When creating a Transfer Request, use the following` peltrans` parameters:
...
<th class="HeadD-Column1-Header1"><p>Value</p>         </th>
   ...
   <tbody>
      <tr>
         <td><p><code>-sw_copy_indicator (-ci)</code></p>
<p>Request copy</p>         </td>
...
<li><code>N</code> = no (default)</li>
</ul>         </td>
      </tr>
      <tr>
         <td><p><code>-sw_auth_indicator (-ai)</code></p>
...
When creating a Trading Partner, use the following `peladm create_tradepart` parameters:
...
<th class="HeadD-Column1-Header1"><p>Value</p>         </th>
   ...
   <tbody>
      <tr>
         <td><p><code>-sw_copy_indicator (-ci)</code></p>         </td>
         <td><p>SWIFTNet File or Message Copy service indicator.</p>
<p>Select this option to request a copy of the file or message to be sent to a third party defined at the service level.</p>         </td>
         <td><p>One of:</p>
<ul>
...
<li><code>N</code> = no</li>
      ...
      <tr>
         <td><p><code>-sw_auth_indicator (-ai)</code></p>         </td>
         <td><p>SWIFTNet Authorization notification indicator.</p>
<p>Select this option to receive Authorization system messages for files or messages sent over Y-copy services.</p>         </td>
         <td><p>One of:</p>
<ul>
...
When creating a Model, use the following` peladm create_model` parameters:
...
`x_sw_auth_by_xfer` (link the original sender transfer with the authorization transfer)

`x_sw_auth_to_xfer` (link SnF InterAct authorization transfer with the original message)
...
Authorization system messages have the` sw_auth_indicator` parameter set.
...
If the file from SWIFT SnF, received in push-mode, contains a Sw:Copy element in the FileDescriptor, the transfer is marked with the `copy_indicator`. The CopyType and CopyState elements are recovered and saved on the transfer, available as end-of-transfer parameters and Decision Rule condition parameters.
...
The ThirdPartyToReceiver information, if present, is deposited in the SWIFT parameter file. The parameter` sw_add_parameters` contains the filename and directory path. For an example of a file containing ThirdPartyToReceiver information, refer to [SWIFTNet dump to XML](../../swiftnet_backup_sites/swiftnet_dump_to_xml).

<span id="GikAPI"></span>

## GikAPI support for File and Message Copy

The following parameters can be found in the `gikapic.h` header:
...
When creating a transfer, use the values `ITP_SWIFTNET_YES` and `ITP_SWIFTNET_NO` (uses `XferRequest_t`).

For `XferUpdate` and `XferGet`, use boolean values {0, 1} for the parameters (uses `XferRecord_t`).
