...
The `pelbase` command enables you to:
...
1.  On the originating Gateway, execute the `pelbase export` command for the object or objects you want to export.  
    The result of each `pelbase export` command is a file describing the objects that you can transfer between machines or versions.  
    ...
    Use the `-output (-f)` parameter to specify the name of the resulting file. If you do not enter a value, Gateway prints the command results on the screen.  
...
2.  On the target Gateway, execute the `pelbase import` command. Execute the command as many times as necessary to import each file you created with the `pelbase export` command.  
    If the objects were exported with passwords encrypted, use the `-encryption_key_file`(`-ekf`) parameter to specify the key used for password encryption.  
    If the objects were exported from a Gateway version which does not support password encryption for exported objects, use the `-import_pwd_cleartext` parameter to import clear-text passwords.  
...
-   [**pelbase export\_site**](#pelbase_export_site)
