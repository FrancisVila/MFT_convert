{
    ...
    "weight": "180"
}This section describes how to register a Transfer CFT instance with either an on premise or SaaS Flow Manager{{< TransferCFT/flowmanager  >}}. You can refer to the [Flow Manager User Guide](https://docs.axway.com/bundle/FlowManager_20_allOS_en_HTML5/page/user_guide.html) for Flow Manager details.

## Prerequisites

****Transfer CFT{{< TransferCFT/componentlongname  >}} prerequisites****
...
- If you are implementing a SaaS cloud Flow Manager, you additionally require:
    -   A Flow Manager Agent capable of interconnecting Flow Manager and your Transfer CFT{{< TransferCFT/componentlongname >}} instance. Refer to the [Flow Manager User Guide](https://docs.axway.com/bundle/FlowManager_20_allOS_en_HTML5/page/user_guide.html) for details.
...
This section describes the steps to register your Transfer CFT{{< TransferCFT/componentlongname  >}} with Flow Manager{{< TransferCFT/flowmanager  >}}. The procedure is the same for either an on-premise or SaaS Flow Manager{{< TransferCFT/flowmanager  >}}, with the exception of the two steps described in [Define the Flow Manager{{< TransferCFT/flowmanager  >}} Agent](#Define).
...
#### Define UCONF parameters used for Transfer CFT{{< TransferCFT/componentlongname  >}} instance identification
...
#### Optionally define a proxy server for on-premise Flow Manager{{< TransferCFT/flowmanager  >}} to Transfer CFT{{< TransferCFT/componentlongname  >}} communication

To use a proxy server for your on-premise Flow Manager{{< TransferCFT/flowmanager  >}} to connect to Transfer CFT{{< TransferCFT/componentlongname  >}}, set the following parameters.
...
To use a proxy server for your Transfer CFT{{< TransferCFT/componentlongname  >}} to connect to Flow Manager{{< TransferCFT/flowmanager  >}}, set the following parameters.
...
#### Import the root certificate for the Flow Manager{{< TransferCFT/flowmanager  >}} certificate
...
1. Download the HTTPS root certificate's CA, which is used to authenticate Flow Manager{{< TransferCFT/flowmanager >}}.
1. Import this root CA into the PKI database using the PKIUTIL PKICER command.
1. Set the `iname `to the root CA path.
1. Define the UCONF variable `cg.ca_cert_id`, which must correspond with the value you set in the previous step. It is required so that Transfer CFT{{< TransferCFT/transfercftname >}} knows which certificate to use to authenticate Flow Manager{{< TransferCFT/flowmanager >}}. Using CFTUTIL:

#### Define the parameters used for the Flow Manager{{< TransferCFT/flowmanager  >}} connection

Set the following parameters that are used to connect to Flow Manager{{< TransferCFT/flowmanager  >}}.
...
You may want to automatically assign an existing Flow Manager{{< TransferCFT/flowmanager  >}} configuration policy during the Transfer CFT{{< TransferCFT/componentlongname  >}} registration. To do so, set the UCONF parameter `cg.configuration_policy` to the name of the desired policy.
...
#### Enable Flow Manager{{< TransferCFT/flowmanager  >}}
...
Start the Transfer CFT{{< TransferCFT/transfercftname  >}} Copilot to automatically trigger registration. From the Flow Manager UI, check the **Product List** to confirm that the registration was successful.
