...
********<span class="autonumber"></span>Registration exchange overview (the same for either {{< TransferCFT/PrimaryCGorUM  >}} or {{< TransferCFT/suitevariablesFlowManager  >}})********
...
- Copilot sends a registration request through a simple authenticated SSL connection and submits its registration. Copilot authenticates the Central Governance or {{< TransferCFT/suitevariablesFlowManager >}} server using the CA certificate pointing by the uconf:cg.ca\_cert\_id parameter. The registration request contains:
...
These changes create two security profiles (CFTSSL) on the {{< TransferCFT/axwayvariablesComponentShortName  >}} (except when using a SAF-based PKI {{< TransferCFT/axwayvariablesComponentShortName  >}}). The profiles are named SSL\_DEFAULT; one profile is of type client and one is of type server. Their SSL version is TLSV1COMP. The configured cipher list is CIPHLIST= ('53','47') for {{< TransferCFT/axwayvariablesComponentShortName  >}} 3.1.2 and 3.1.3. The values represent the following cipher suites:
