...
- If the server sends an **invalid credential** reply, check that the UCONF `copilot.restapi.authentication_method` and `am.type` parameters are consistent with the **REST API server authentication method** diagram. Please see <a href="../../../c_intro_userinterfaces/web_copilot_ui#Authentication_methods" class="MCXref xref">Authentication methods</a>
