...
| ABORT |   | {{< TransferCFT/componentshortname  >}} transfer abort request |
...
| NONE |   |   |
| CP nn% |   | Transfer performed with the compression option. The compression rate is then displayed. This rate expresses the number of bytes to be sent in relation to the number of bytes actually sent. The number of bytes to be sent may be different from the number of bytes in the file if the {{< TransferCFT/componentshortname  >}} truncates or pads records, depending on the parameter settings |
...
| ERR LREC |   | Error sending or receiving the file data. {{< TransferCFT/componentshortname  >}} detects an invalid length for the data read or to be written |
...
| NO PARM |   | Error due to incorrect {{< TransferCFT/componentshortname  >}} parameter settings |
...
| NO TURN | ODETTE | The {{< TransferCFT/componentshortname  >}} cannot hand over to the partner (CD) as the partner handed over during the previous exchange |
...
| NPART |   | REQUESTER partner mismatch with SERVER partner in SIT profile. Strict naming and consistency rules are imposed both by {{< TransferCFT/componentshortname  >}} and by the PESIT standard. |
...
| RTO |   | {{< TransferCFT/componentshortname  >}} time-out during the transfer phase. This time-out corresponds to the RTO parameter of the CFTPROT command |
...
| SYPOST |   | Communication error between {{< TransferCFT/componentshortname  >}} and the directory EXIT task |
