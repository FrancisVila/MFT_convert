...
[CFTACCNT.](../../CFTUTIL/Conf/CFTACCNT.htm)
...
-   Recording data
    ...
    in <span>Transfer CFT</span> files. When the primary file is full, <span>Transfer CFT</span>
...
<span>Transfer CFT</span> authorizes only one identifier of the CFTACCNT type.
...
-   Recording of data
    ...
    in <span>Transfer CFT</span> files. In this case, the CFTACCNT command defines the names of the files receiving
...
mode, see [Recording mode TYPE](#Recordin). For each terminated transfer, <span>Transfer CFT</span> records the information
...
The <span>Transfer CFT</span> statistical

file is full when the maximum number of records, the MAXREC parameter,

is reached. When this happens, the <span>Transfer CFT</span> switches to an alternate
...
<span>Transfer CFT</span> requires at least 1 empty statistical file at the time
...
The statistical files must be created before the <span>Transfer CFT</span> is activated.

This operation is performed in the command CFTFILE TYPE = ACCNT.



When you shut down <span>Transfer CFT</span>, using the SHUT command, <span>Transfer CFT</span>
...
the <span>Transfer CFT</span> application via the utility.
