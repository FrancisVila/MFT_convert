...
<span>Transfer CFT</span> provides a set of services to track activity related to transfers and their status as well as evaluating the system health.
...
2.  To activate the new values, run the command: CFTUTIL reconfig type = uconf
    ...
    -   If <span>Transfer CFT</span> is stopped when setting uconf values, you do not need to execute the reconfig command.
...
The message <span>CFTC30W Catalog Alert cleared : **level=67%** </span>indicates that the catalog is sufficient. If it were not, the catalog would be extended again at next alert in TLVWRATE seconds.
...
If you are using the <span>cft.cftcat.auto\_expand</span> parameter in a z/OS environment, refer to the <span>SHARECAT </span>parameter in the *Installation and Operation Guide* for OS specific details.
...
For information on the RECONFIG command, please see <a href="../../admin_commands_intro/reconfig">Manage configuration updates - RECONFIG</a>.
