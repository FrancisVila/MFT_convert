...
#### <span id="Acceptance_by_CFT_"></span>Acceptance by Transfer CFT:
...
#### <span id="Availability_for_the_monitor_"></span>Availability for Transfer CFT:
...
several instances of Transfer CFT via a single catalog file.
...
Programming interface used to integrate Transfer CFT into applications
...
Command used to create or delete Transfer CFT files (parameter, partner,
...
Main Transfer CFT task used to schedule transfers. It controls
...
Batch or line mode tool giving access to all Transfer CFT functions
and parameters.
...
Instructions used in Transfer CFT to configure the monitor and working
...
and conversion mode, can be internal to Transfer CFT or created by the
...
Transfer CFT simulates a synchronization point for each credit message
sent or received.
...
#### <span id="Data_compacting_"></span>Data compacting:
...
#### <span id="Data_compression_"></span>Data compression:

Action whereby the volume of data sent and therefore the transfer times are
reduced.

#### <span id="Data_integrity_"></span>Data integrity:
...
#### <span id="Directory_exit_"></span>Directory exit:
...
#### <span id="Dynamic_partner_"></span>Dynamic partner:
...
#### <span id="EERP__End_to_End_Response__acknowledgement_"></span>EERP (End to End Response) acknowledgement:
...
#### <span id="End_of_transfer_procedure_"></span>End of transfer procedure:
...
#### <span id="Environment_variable_"></span>Environment variable:
...
#### <span id="Error_procedure_"></span>Error procedure:
...
#### <span id="File_exit_"></span>File exit:
...
#### <span id="Horizontal compression:"></span>Horizontal compression:
...
#### <span id="Identification"></span>Identification:
...
#### <span id="IDTU__catalog_identifier__"></span>IDTU (catalog identifier):

Unique identifier assigned to a transfer by Transfer CFT. It is used
in local transfer monitoring and catalog list commands.

#### <span id="Implicit_send_"></span>Implicit send:

Mechanism used to make a file available on a permanent basis (CFTSEND
IMPL=YES). See also Availability for the partner.

#### <span id="Integrity_checking_"></span>Integrity checking:
...
#### <span id="List_exit_"></span>List exit:

File exit used by remote partners to query the Transfer CFT catalog
on a host.
...
in a specific transfer. In Transfer CFT and for given protocols, two type
of messages can be sent: simple messages or reply messages.

#### <span id="Model_file_"></span>Model file:

Each file transfer is processed by Transfer CFT in line with a model
...
#### <span id="Monitor__transfer_or_CFT_monitor__"></span>Monitor ( Transfer CFT):
...
Process whereby Transfer CFT activities are controlled by querying the catalog
(transfer properties and state) and the log file (chronological history
of all events in information or error message form).

##### N

#### <span id="Network_resource_"></span>Network resource:
...
#### <span id="Partner"></span>Partner:

Logical entity, to which Transfer CFT can send data and from which it
...
#### <span id="PeSIT__Protocole_d_Echanges_pour_un_Syst_me_Interbancaire_de_T_l___compensation__"></span>PeSIT (Protocole d'Echanges pour un Système Interbancaire de Télé- compensation):
...
#### <span id="PI__Parameter_Identifier__"></span>PI (Parameter Identifier):
...
#### <span id="Reading_or_read_mode_transfer_"></span>Reading or read-mode transfer:
...
#### <span id="Reply_message_"></span>Reply message:
...
#### <span id="Symbolic_variable_"></span>Symbolic variable:

Transfer CFT variable representing a data item associated with the transfer
and for which the value will only be known when the transfer is executed
and not when the parameters are set.

#### <span id="Synchronization_interval_"></span>Synchronization interval:
...
#### <span id="Synchronization_point_setting_"></span>Synchronization point setting:
...
#### <span id="Transfer_restart_"></span>Transfer restart:
...
#### <span id="Transfer_resumption_"></span>Transfer resumption:

Means whereby a suspended transfer can be resumed either automatically
or manually.

#### <span id="Transfer_scheduling_"></span>Transfer scheduling:
...
-   D:
    ...
    and will be triggered automatically as soon as the Transfer CFT resources
