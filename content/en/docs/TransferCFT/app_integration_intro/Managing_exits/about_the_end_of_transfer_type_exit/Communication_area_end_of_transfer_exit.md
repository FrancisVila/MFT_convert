...
Transfer identifier
idf
File identifier (if  TYPE = FILE)
idm
Message identifier (if TYPE= MESSAGE)
ida
Application identifier
part
Partner name
direct
Transfer direction (S/R)
diagi
Internal diagnostics code
diagp
Protocol diagnostics code
```
...
Data code (A, E or B)
fkeylen
Key length (indexed file)
fkeypos
Key position (indexed file)
flrecl
Record length
fname
...
nidf
Network identifier of the file
npart
Remote partner
nfname
Network name of the file
nkeylen
Network key length (indexed)
nkeypos
Network key position (indexed)
```
...
version
Exit version
The value of this parameter is used to identify the version
of the EXIT task supplied.
comment
Local comment
This field can be modified by the user program. Any modifications
are taken into account when the catalog is updated (if usraction
= UPDATE with or without change of state).
state
State requested on UPDATE (D, H, K, X). See [Transfer
states.](../#Transfer_state)
```
...
EXIT identifier
parmexit
User EXIT parameter (PARM parameter of the CFTEXIT command)
language
Language:
