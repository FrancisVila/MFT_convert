...
parameters that Transfer CFT{{< TransferCFT/componentshortname  >}} needs to establish network and protocol connections.
...
When Transfer CFT{{< TransferCFT/componentshortname  >}} does not know the partner, the following fields are
empty:
...
field of the Transfer CFT{{< TransferCFT/componentshortname  >}} catalog.

The content of the diag field appears with the appropriate error message
if the return code is not 0 and 1.

If the msg field is defined, its content is sent to the Transfer CFT{{< TransferCFT/componentshortname  >}}
standard output.
