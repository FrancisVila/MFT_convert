{
    ...
    "weight": "390"
}# <span id="Title"></span>Use the communication structure
...
remote site, and after the record is read if <span>Transfer CFT</span> manages file
...
parameter before handing back control to <span>Transfer CFT</span>.
...
record is written if <span>Transfer CFT</span> manages file accessing. If file accessing

is managed by the user function, the latter has to write the record before

handing back control to <span>Transfer CFT</span>.
...
sender end, records are compressed before being sent by the <span>Transfer CFT</span>. At the receiver end, the

records are decompressed by the <span>Transfer CFT</span> immediately after

they are received. The records <span>Transfer CFT</span> supplies to the user function
