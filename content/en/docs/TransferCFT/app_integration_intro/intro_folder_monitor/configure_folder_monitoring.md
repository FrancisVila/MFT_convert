...
1.  Activate the folder monitoring option.
    ...
    -   Set uconf parameter folder\_monitoring.enable to <span>Yes</span>.
...
-   The files are sent to a given partner, <span>newyork</span>, using an IDF name of IDFA.
...
The files to be sent must be moved to the directory that corresponds to the destination partner name, for example <span>/home/CFT/fm/dir\_b/newyork </span>for the partner named <span>newyork</span>.
...
The files to be sent must be moved to the directory that corresponds to the destination partner and idf names, for example /home/CFT/fm/dir\_c/newyork/idf1 for the partner <span>newyork </span>and idf <span>idf1</span>.
...
For example, you have a file named <span>A0001.appli1.cft.XXX</span> in the <span>scan1 </span>folder, and you have the following two objects in your configuration:
...
-   For more information on how to effectively use separators with symbolic variables, please see [Separate fields in symbolic variables.](../../CFTUTIL/Parameter_index/symbolic_variables.htm#Separate)

-   For more information on the various symbolic variables to use in CFTSEND with CFTFOLDER, see [List of symbolic variables.](../../CFTUTIL/Parameter_index/symbolic_variables.htm#List_of_symbolic_variables)
...
3.  Depending on the folder monitoring method you have enabled, the file is picked up and moved to the working directory.

4.  ![](folder7.png)

5.  The file transfer and any related processing occur and the file is then moved to the storage folder.
...
-   A working folder, for example: <span>mkdir myWorkFolder</span>
...
-   Stop <span>Transfer CFT</span> prior to start the procedure, enter: `cft stop `
...
7.  Start Transfer CFT: <span>cft start</span>



8.  Put the test file in the <span>runtime/MyScanFolder</span> folder.



9.  Navigate to the `runtime/MyArchivedFolder` and check that the <span>MyFile.txt</span> is stored there.



10. Check in the log for a message similar to the following:
...
4.  Create a SEND model using `archivefname`. In this example the transfer's IDTU is appended on the filename:
...
5.  Activate the new CFTFOLDER object:
...
8.  Start Transfer CFT: <span>cft start</span>



9.  Put the test file in the <span>runtime/MyScanFolder</span> folder.



10. Navigate to the `runtime/MyArchiveFolder` and check that the <span>MyFile.txt</span> is stored there.



11. Check in the log for a message similar to the following:



12. <table data-cellspacing="0">
<tbody>
<tr>
...
The following example demonstrates how to use the [USERCTRL](../../CFTUTIL/Parameter_index/userctrl.htm) parameter to define access control for another user. See the sections on user rights in <a href="../../../cft_intro_install/unix_install_start_here/run_first_time_ux/run_first_time_ux/t_adding_system_user_unix">Using system users - UNIX</a> or <a href="../../../cft_intro_install/windows_install_start_here/windows_install_start_here/running_cft_for_the_first_time_windows/add_system_user_windows">How to enable system users - Windows</a> for details.
...
3.  Start Transfer CFT with `usercft `as the user.



4.  User1 creates the <span>/home/user1/scandir\_app1</span> and `/home/user1/workdir` folders:
...
<tbody>
<tr>
<td>mkdir /home/user1/scandir_app1
...
6.  `App1 `writes files to this specific scandir folder, e.g. `/home/user1/scandir_app1`, which belongs to user1. Notice that the usercft user does not have access to`  scandir_app1`.



7.  From CFTUTIL, create a CFTFOLDER:
...
9.  Activate the CFTFOLDER object:
...
11. Create a file called<span> Myfile.txt</span>, and copy it to the `/home/user1/scandir_app1` folder.
