...
You can use the<span> Folder monitoring migration</span> **utility** to help you migrate from the UCONF method of folder monitoring to the CFTFOLDER method. This procedure may be useful if you have migrated or upgraded from Transfer CFT 3.1.2 to Transfer CFT 3.2.4.
...
Using the <span>cftmifm</span> migration command creates a text file that the CFTUTIL utility can use to migrate the folder definitions, where the CFTFOLDER definitions are an extraction from the UCONF definitions.
...
2.  <span>cftmifm migrate -o fm1.cfg</span>

3.  Save uconf folder configuration:

4.  <span>CFTUTIL CFTEXT type=uconf,id=folder\_monitoring.\*,fout=fm\_uconf\_save.cfg</span>

5.  Interpret fm1.cfg file:

6.  <span>CFTUTIL config type=input,fname=fm1.cfg</span>

7.  Purge uconf folder configuration:

8.  <span>cftmifm purge</span>
...
2.  <span>cftmifm migrate -p A\* -o fm2.cfg</span>

3.  Make a backup save of the UCONF folder configuration:

4.  <span>CFTUTIL CFTEXT type=uconf,id=folder\_monitoring.\*,fout=fm\_uconf\_save.cfg</span>

5.  Interpret the generated fm2.cfg file:

6.  <span>CFTUTIL config type=input,fname=fm2.cfg</span>

7.  Purge the UCONF folder configuration:

8.  <span>cftmifm purge -p A\*</span>
...
<tbody>
<tr>
<td><p>CALL PGM(CFTMIFM) PARM('migrate' '-o' 'CFTPROD/fm1cfg')</p>
<p>CALL PGM(CFTUTIL) PARM(CFTEXT 'type=uconf,id=folder_monitoring.*,fout=CFTPROD/fm_uconf')</p>
<p>CALL PGM(CFTUTIL) PARM(CONFIG 'type=input,fname=fm1cfg')</p>
<p>CALL PGM(CFT324CI/CFTMIFM) PARM('purge')</p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr>
<td><p>CALL PGM(CFTMIFM) PARM('migrate' '-p' 'A*' '-o' 'CFTPROD/fm2cfg')</p>
<p>CALL PGM(CFTUTIL) PARM(CFTEXT 'type=uconf,id=folder_monitoring.*,fout=CFTPROD/fm_uconf')</p>
<p>CALL PGM(CFTUTIL) PARM(CONFIG 'type=input,fname=fm2cfg')</p>
<p>CALL PGM(CFTMIFM) PARM('purge' '-p' 'A*')</p></td>
</tr>
</tbody>
</table>
...
1.  Extracts all folder definitions: <span>cftmifm migrate -p \* -o &lt;temp\_file&gt;</span>

2.  Print the output.

3.  Apply the extracted definitions: <span>CFTUTIL config type=input,fname=&lt;temp\_file&gt;</span>

4.  Purges the all existing UCONF folder definitions: <span>cftmifm purge -p \*</span>
...
A prerequisite to performing a rollback is that you must have made a backup of the UCONF folder configuration prior to having migrated. Using the example above, this is the step: <span>CFTUTIL CFTEXT type=uconf,id=folder\_monitoring.\*,fout=fm\_uconf\_save.cfg</span>



1.  Interpret the backup uconf folder file, for example <span>fm\_uconf\_save.cfg</span>.

2.  <span>CFTUTIL config type=input,fname=fm\_uconf\_save.cfg</span>

3.  Manually remove all CFTFOLDER objects:

4.  <span>CFTFOLDER ID=&lt;folder&gt;,mode=delete</span>
...
<p><a href="../folder_monitor_uconf">More information</a></p></td>
<td>Boolean</td>
<td>No</td>
<td>No</td>
...
-   <a href="#">Migrate to CFTFOLDER folder monitoring</a>
