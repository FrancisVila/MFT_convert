{
    ...
    "weight": "230"
}Before you can start Transfer CFT{{< TransferCFT/componentshortname  >}} from the Transfer CFT{{< TransferCFT/componentshortname  >}} Copilot server, the Copilot server must be started. Additionally you will need rights to log on to this server. The overall process requires that you:

- [Define rights before starting the Transfer CFT{{< TransferCFT/componentshortname >}} UI server (Copilot)](#Define%20rights%20before%20starting%20the%20CFT%C2%A0Navigator%20server)
- [Define rights before logging on the Transfer CFT{{< TransferCFT/componentshortname >}} UI server (Copilot)](#Define%20rights%20before%20logging%20on%20the%20CFT%C2%A0Navigator%20server)
- [Define rights before starting Transfer CFT{{< TransferCFT/componentshortname >}}](#Define_rights_before_starting_Transfer_CFT)
- [Define a domain user](#Define%20domain%20user)
- [Define folder rights](#Define)
- [Define system user access](#Define2)

To start the Transfer CFT{{< TransferCFT/componentshortname  >}} Copilot server you must give each Windows user read/write rights for the Transfer CFT{{< TransferCFT/componentshortname  >}} installation folder.
...
- no: Any user who logs on the Transfer CFT{{< TransferCFT/componentshortname >}} UI server will have their processes identified as the user who started the Transfer CFT{{< TransferCFT/componentshortname >}} Copilot server.

<!-- -->

- yes: Any user who logs on the Transfer CFT{{< TransferCFT/componentshortname >}} UI server will have their processes identified as their own.
...
Transfer CFT{{< TransferCFT/componentshortname  >}} supports domain user accounts, which allows a service to use Windows service security features.
...
To be able to start the Transfer CFT{{< TransferCFT/componentshortname  >}} server and the Transfer CFT{{< TransferCFT/componentshortname  >}} UI, you must give each user read and write rights for Transfer CFT{{< TransferCFT/componentshortname  >}}.
...
If copilot.misc.createprocessasuser=yes in UCONF , or Createprocessasuser=yes in \[MISC\], the user starting the Transfer CFT{{< TransferCFT/componentshortname  >}} Copilot server must do the following tasks to allow other users to log on. Additionally, those users must exist in the Windows system users list.
