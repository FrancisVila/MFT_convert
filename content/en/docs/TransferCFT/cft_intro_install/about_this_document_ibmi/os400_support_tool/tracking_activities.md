...
-   Transfer CFT LOG, a file designated by the CFTLOG configuration command \[A\]FNAME parameter. This file contains, for example, , and
...
-   Message queue that receives specific incident messages from the interface functions; by default, the function associated with the user (same as NOTIFY='\_\_\_\_\_\_')
...
-   JOBLOG ( message queue and QPJOBLOG spool file), LOGCLPGM parameter in the \*JOBD:
...
-   LOG (QHST) which records job start and end times, status changes for network objects and so on
