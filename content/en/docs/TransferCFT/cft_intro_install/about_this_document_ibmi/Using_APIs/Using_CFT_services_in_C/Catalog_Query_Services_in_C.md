...
`CAPI_FUNC_UNDEF`: Command not valid or service refused by the operating security
system
...
`CAPI_CAT_ALLOC`: cftai only, Catalog file allocation error
...
`CAPI_CAT_SELECT`: cftaix only, Selection incorrect
