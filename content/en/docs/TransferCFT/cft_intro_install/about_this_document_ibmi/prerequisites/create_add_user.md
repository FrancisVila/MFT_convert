...
The system environment of the user who performs the installation procedure is applied by default if there is no Transfer CFT-specific profile applied during installation. Notice that the user profile owns the objects created during installation, but not restored objects such as programs, commands, etc.
...
{{< TransferCFT/axwayvariablesComponentLongName  >}} supplies two related commands, CHGOWNCFT and GRTOBJCFT, with the library and profile name as call parameters.

- CHGOWNCFT modifies the ownership of an object or directory.
- GRTOBJCFT modifies the authority of all the objects in a library for the selected user, with the following actions occurring for each object:
    -   The current authority is revoked.
    -   The \*ALL authority is given to the selected user.
    -   The \*USE authority is given the to the \*PUBLIC users.

To grant a user the permission to use Transfer CFT at the system level, enter the following command to assign user rights:
...
You must give \*RX rights to all objects created in the `/home/cft/Transfer_CFT/install` (default) and `/home/cft/Transfer_CFT/runtime` (default) directories.
