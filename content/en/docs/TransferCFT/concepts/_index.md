...
file. In the absence of these commands for a given transfer, Transfer CFT uses the default CFTSEND and CFTRECV command parameters, which are
valid regardless of the IDF.

When using Central Governance with Transfer CFT, an identifier corresponds to each flow definition. In the absence of this flow identifier, Transfer CFT uses the flow default value.
...
### <span id="Messages__IDM"></span>Messages: IDM

A message is a character string specified by a SEND command and sent
by the Transfer CFT as a specific transfer. A message transfer does not make reference to a flow definition (a CFTSEND or CFTRECV command).
...
#### <span id="Transfer_identifier__IDT"></span>Transfer identifier: IDT
...
#### <span id="Catalog_identifier__IDTU"></span>Catalog identifier: IDTU
...
so on. When there are several Transfer CFTs on the same computer, uniqueness
is guaranteed between two Transfer CFTs sharing the same catalog.
