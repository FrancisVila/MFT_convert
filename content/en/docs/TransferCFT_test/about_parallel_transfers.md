...
-   Client and server recommendations that demonstrate settings for a high volume of transfers
-   Connection / maximum transfer scenarios with output
-   Session related scenarios with output
...
This parameter controls the number of CFTTFIL processes that can run. Since more processes use more machine memory, remember when increasing MAXTASK to take available resources into account. See also MAXTASK values.
...
This parameter defines the number of transfers managed per task, and operates similar to load balancing between tasks. See also TRANTASK values.
