...
10. In the *End user Transfers API* settings pane:  
    1.  Select **Allow this account to submit transfers using the Transfers RESTful API** to enable calls from the {{< SecureTransport/componentshortname >}} REST file transfer API authenticated with the credentials from accounts in the business unit. When this option is selected, the account will be allowed to trigger server initiated transfers using the Transfers RESTful API resource and retrieve the tracking information for these transfers.
...
11. In the *AdHoc Settings* pane:  
    1.  Select **Login by email** to allow users in the business unit to log in using the value of the **Email Contact** field as well as the **Login Name**. A user of one of the {{< SecureTransport/companyname >}} Email Plug-ins must be able to login by email.  
...
19. Select **Allow Login Restriction Policy modifying** to enable modifying of the Login Restriction Policy at the account level.  

In the *Bandwidth limits* pane:  

1.  Select a **Bandwidth Limits Policy** to apply:  
