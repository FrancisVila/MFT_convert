...
<table cellpadding="0" cellspacing="0">
   ...
   <col/>
      <tr>
         ...
         <td data-mc-autonum="&lt;b&gt;Note&lt;/b&gt;" valign="top">A cluster with cluster mode <code>passive_legacy</code> does not monitor the state of each server in the cluster. However, if you attempt to synchronize all the servers in the cluster, and no errors occur, it indicates that both servers in the cluster are up.         </td>
      </tr>
</table>
...
<table cellpadding="0" cellspacing="0">
   ...
   <col/>
      <tr>
         ...
         <td data-mc-autonum="&lt;b&gt;Caution  &lt;/b&gt;" valign="top">In an active/passive cluster, the secondary server cannot be bounced remotely.          </td>
      </tr>
</table>
...
## <span id="Restore"></span>Restore a server to an active/passive cluster

1.  Start the Administration Tool server and the TM server on the restored computer.
2.  On the primary server, perform a manual synchronization from the Administration Tool. For instructions, see [Synchronize the cluster from the primary server](../c_st_standard_cluster_synchronization).
...
<table cellpadding="0" cellspacing="0">
   ...
   <col/>
      <tr>
         ...
         <td data-mc-autonum="&lt;b&gt;Note&lt;/b&gt;" valign="top">It is recommended that you synchronize the restored computer as soon as possible. If the restored computer becomes primary before it is synchronized, it might process messages based on outdated data it received before it failed.         </td>
      </tr>
</table>

## <span id="Remove"></span>Remove a server from an active/passive cluster
...
## <span id="Manual"></span>Manual failover
