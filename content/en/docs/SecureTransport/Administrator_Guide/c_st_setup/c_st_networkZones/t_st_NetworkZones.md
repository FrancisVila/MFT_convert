...
4.  Under *Streaming Configuration*, enter ports for the TM Server to connect to for each enabled protocol server and for the Administration Tool server.  
    In the special `Private` network zone on {{< SecureTransport/componentshortname >}} Server, these settings define both the ports that the TM Server connects to and the ports that the protocol servers and the Administration Tool server listen on for connections from the TM Server.
...
## Create a network zone to define communications with {{< SecureTransport/componentshortname  >}} Edge servers

Each network zone can define the communications between the TM Server on a {{< SecureTransport/componentshortname  >}} Server and one or more {{< SecureTransport/componentshortname  >}} Edge servers. If different protocol servers run on different {{< SecureTransport/componentshortname  >}} Edge servers, you can control which protocol servers on which {{< SecureTransport/componentshortname  >}} Edge servers the TM Server connects to by specifying different protocol servers and different {{< SecureTransport/componentshortname  >}} Edge servers in different nodes.
...
7.  If the SOCKS5 proxy is enabled in this network zone and DNS is not available on this {{< SecureTransport/componentshortname >}} Server, select **Use the Edge DNS configuration** to resolve transfer site host names and FQDNs using the DNS on the {{< SecureTransport/componentshortname >}} Edge so that {{< SecureTransport/componentshortname >}} can route transfers correctly.
...
15. In the **Address** field, enter the IP address of a {{< SecureTransport/componentshortname >}} Edge server that runs the protocol servers configured for this network zone node.

16. Repeat step 10 and 11 for each {{< SecureTransport/componentshortname >}} Edge server in this network zone node.
...
18. Click **Save** to save the network zone.  
    {{< SecureTransport/componentshortname >}} save the network zone in the database.
...
Use network zones to specify how {{< SecureTransport/componentshortname  >}} connects to the {{< SecureTransport/securetransportedgename  >}} SOCKS5 proxy (or a HTTP proxy, where relevant) for server-initiated file transfers. You can use the same network zone node to define streaming and proxy configuration or you can define only streaming or only proxy configuration in a node or network zone.
...
## Specify TM Server communication ports and IP address for protocol servers on {{< SecureTransport/componentshortname  >}} Edge

The `Private` network zone is the only network zone defined on the {{< SecureTransport/componentshortname  >}} Edge server. You cannot delete it or define more network zones.
...
You can use SSL to secure the communication between the TM server running on {{< SecureTransport/componentshortname  >}} Server and the protocol servers running on {{< SecureTransport/componentshortname  >}} Edge.

In a streaming deployment, the protocol servers on {{< SecureTransport/componentshortname  >}} Edge have the role of server because the TM server on {{< SecureTransport/componentshortname  >}} Server connects to them and the TM server on {{< SecureTransport/componentshortname  >}} Server has the role of client because it connects to the protocol servers on {{< SecureTransport/componentshortname  >}} Edge.
...
2.  On {{< SecureTransport/componentshortname >}} Edge:
...
3.  On {{< SecureTransport/componentshortname >}} Server:
    ...
    4.  In the network zone you created to define communications with {{< SecureTransport/componentshortname >}} Edge servers, open the *Edit Network Node* page and, to secure the SSL communication for a protocol, select the **SSL Key Alias** you specified when you imported the certificate in step c.

## Specify the {{< SecureTransport/componentshortname  >}} Edge load-balancing policy

When a client establishes a user session to a protocol server running on {{< SecureTransport/componentshortname  >}} Edge, the load-balancing policy that {{< SecureTransport/componentshortname  >}} Edge uses to allocate a Transaction Manager connection to that session is specified by the `Streaming.LoadBalancingPolicy` server configuration parameter. The valid values are:

-   **`Round-Robin` –** {{< SecureTransport/componentshortname >}} Edge directs connections to connected Transaction Managers in a circular sequence and allocates the least-used connection to that Transaction Manager to the session.
-   **`Random` –** {{< SecureTransport/componentshortname >}} Edge randomly selects a connection from all the connections to all the connected Transaction Managers and allocates it to the session.
-   **`Blacklist-Round-Robin` –** {{< SecureTransport/componentshortname >}} Edge directs connections to connected Transaction Managers in a circular sequence and allocates the least-used connection (but filters bad connections) to that Transaction Manager to the session. `Blacklist-Round-Robin` is the default and recommended value.
-   **`Blacklist-Random` –** {{< SecureTransport/componentshortname >}} Edge randomly selects a connection from all the connections ( but filters bad connections) to all the connected Transaction Managers and allocates it to the session.

In all cases, the user session uses the allocated connection until it terminates.

<span id="Specify"></span>

## Specify allowed {{< SecureTransport/componentshortname  >}} Servers on {{< SecureTransport/componentshortname  >}} Edge

The SOCKS5 proxy on a {{< SecureTransport/componentshortname  >}} Edge server accepts connections for server-initiated transfers only from the {{< SecureTransport/componentshortname  >}} Servers listed. There is no HTTP proxy on the {{< SecureTransport/componentshortname  >}} Edge server. The Transaction Manager on any {{< SecureTransport/componentshortname  >}} Server can connect to a protocol server on any {{< SecureTransport/componentshortname  >}} Edge server.

1.  Select **Setup > Allowed ST Servers**.  
    A page is displayed with the server list.
2.  Under *ST Servers*, list the host names or IP address of the {{< SecureTransport/componentshortname >}} Servers that are allowed to connect to this {{< SecureTransport/componentshortname >}} Edge server, either to a protocol server or to a SOCKS5 or HTTP proxy.
3.  Click **Update**.
4.  Restart the SOCKS proxy on the {{< SecureTransport/componentshortname >}} Edge server.
