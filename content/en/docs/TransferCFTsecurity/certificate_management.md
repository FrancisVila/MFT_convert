...
![](/Images/TransferCFTsecurity/sec_legend.png)
...
- Local private keys use AES-256 encryption if the `crypto.key_fname` parameter is defined (otherwise Triple DES encryption is used), and are stored in the Transfer CFT internal PKI database. See the *UCONF parameters* section for details on `crypto.key_fname`.
...
The web browser single sign-on, SSO, profile enables users to use the same logging details for {{< TransferCFTsecurity/componentlongname  >}} as other Axway products (for example, Flow Manager). See the *[Transfer CFT User Guide](https://docs.axway.com/bundle/TransferCFT_38_UsersGuide_allOS_en_HTML5/page/Content/AxwayStartPage.htm)* for details and limitations.
